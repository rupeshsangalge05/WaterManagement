...
// NOTE: this form previously registered the submitter's name under the
// key "name". Every other place that reads complaints back
// (AllComplaints.jsx, GiveVotes.jsx) accesses `c.fullName`. If your
// backend schema field is genuinely `fullName`, the old form was
// sending the wrong key and the name likely came back blank on every
// complaint submitted through it. This assumes `fullName` is correct
// based on majority usage — confirm against your Mongoose schema
// before trusting this fix; if the schema field is actually `name`,
// revert this and fix the *other* two files instead.
const ComplaintManager = () => {
  const {
    register,
    handleSubmit,
  ...
  const fetchComplaints = async () => {
    try {
      const { data } = await api.get('/user/complaintForUser');
      setComplaints(Array.isArray(data) ? data : []);
    } catch (err) {
      setError('Failed to fetch complaints');
    } finally {
  ...
  useEffect(() => {
    if (editComplaint) {
      reset({
        fullName: editComplaint.fullName,
        email: editComplaint.email,
        subject: editComplaint.subject,
        message: editComplaint.message
      });

      setPhotoPreview(editComplaint.photo ? `${api.defaults.baseURL}/uploads/complaints/${editComplaint.photo}` : null);
      setVideoPreview(editComplaint.video ? `${api.defaults.baseURL}/uploads/complaints/${editComplaint.video}` : null);
    }
  }, [editComplaint, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editComplaint) {
        await api.put(`/user/complaint/${editComplaint._id}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setSuccess('Complaint updated!');
      } else {
        await api.post(`/user/complaint`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setSuccess('Complaint submitted!');
      }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this complaint?')) return;
    try {
      await api.delete(`/user/complaint/${id}`);
      setSuccess('Complaint deleted!');
      fetchComplaints();
    } catch {
      setError('Delete failed.');
        ...
        <Form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data" className="border p-4 rounded mb-5">
          <Row>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>Name</Form.Label>
                {/* was register('name', ...) — renamed to match the
                    read-side field key used elsewhere */}
                <Form.Control {...register('fullName', { required: true })} isInvalid={!!errors.fullName} />
                {errors.fullName && <small className="text-danger">Required</small>}
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>Subject</Form.Label>
                <Form.Control {...register('subject', { required: true })} isInvalid={!!errors.subject} />
                {errors.subject && <small className="text-danger">Required</small>}
              </Form.Group>
            </Col>
          </Row>
          ...
          <Form.Group className="mb-3">
            <Form.Label>Message</Form.Label>
            <Form.Control as="textarea" rows={3} {...register('message', { required: true })} isInvalid={!!errors.message} />
            {errors.message && <small className="text-danger">Required</small>}
          </Form.Group>
              ...
              <Col md={4} key={c._id}>
                <Card style={cardBaseStyle}>
                  <Card.Body>
                    ...
                    <div className="mt-3">
                      <Button size="sm" variant="info" className="me-2" onClick={() => setEditComplaint(c)}>
                        Edit
                      </Button>
